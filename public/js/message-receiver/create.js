document.addEventListener('DOMContentLoaded', function() {
    const form = document.getElementById('formCreateMessageReceiver');
    const btnCreate = document.getElementById('btn-submit-message-receiver');
    ...
    btnCreate.addEventListener('click', async function(event) {
        ...
        try {
            const response = await axios.post('/message-receiver/store', formData);
            if (response.data.type === 'success') {
                toastr.success(response.data.message);
                unBlockUi();
                form.reset();
                $('#datatableMessageReceiver').DataTable().ajax.reload();
                $('#modalCreateMessageReceiver').modal('hide');
                return;
            }
