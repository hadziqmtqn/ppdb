        ...
        try {
            ...
            if (error.response.status === 422) {
                ...
                for (const key in errors) {
                    ...
                    } else if (key.startsWith('school_report_semester')) {
                        // Tangani error untuk school_report_semester[]
                        const selectInput = form.querySelector(`[name="school_report_semester[]"]`);
                        if (selectInput) {
                            ...
                            const select2Container = $(selectInput).next('.select2-container');
                            if (select2Container.length) {
                                select2Container.addClass('is-invalid');
                                select2Container.after(errorMessage);
                            } else {
